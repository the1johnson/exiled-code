...
<body>";
	...
	Validation::validatePnum($pnum);
	Validation::validateAddress($address);
	Validation::validateName($city);
	Validation::validateName($state);
	Validation::validateZip($zip);
	ShippInfoDAO::updateInfo($fname, $lname, $pnum, $address, $city, $state, $zip, $shippId);
	header("location: ../view/cart_out.php");
	echo "</body></html>";
?>